...
# -*- coding:utf-8 -*-
from models import model_one_class, dense201_hirar, dense201_hirar_6same20, dense201_hirar_new, dense201_hirar_sky 
...
def process_batch_sky(lines,img_path,train=True):
    num = len(lines)
    batch = np.zeros((num,224,224,3),dtype='float32')
    x1_labels = np.zeros(num,dtype='int')
...
def generator_train_batch_sky(train_txt,batch_size,num_classes,img_path):
    ff = open(train_txt, 'r')
    lines = ff.readlines()
    num = len(lines)
    class_6 = num_classes
    ...
    while True:
        ...
        for i in range(int(num/batch_size)):
            a = i*batch_size
            b = (i+1)*batch_size
            x_train, x1_labels = process_batch_sky(new_line[a:b],img_path,train=True)
            x = preprocess(x_train)
            #x = x_train
            y1 = np_utils.to_categorical(np.array(x1_labels), class_6)
...
def generator_val_batch_sky(val_txt,batch_size,num_classes,img_path):
    f = open(val_txt, 'r')
    lines = f.readlines()
    num = len(lines)
    class_6 = num_classes
    ...
    while True:
        ...
        for i in range(int(num / batch_size)):
            a = i * batch_size
            b = (i + 1) * batch_size
            y_test,y1_labels = process_batch_sky(new_line[a:b],img_path,train=False)
            x = preprocess(y_test)
            #y = np_utils.to_categorical(np.array(y_labels), num_classes)
            y1 = np_utils.to_categorical(np.array(y1_labels), class_6)
            test_data = x
...
def main():
    ...
    path = '/content/drive/MyDrive/'#'home/your_path/'
    img_path = path+'YogaVids_Frames/'
    train_file = path+'yoga_train_sky.txt'
    test_file = path+'yoga_test_sky.txt'
    ...
    model.fit_generator(generator_train_batch_sky(train_file, batch_size, num_classes,img_path),
                          steps_per_epoch=train_samples // batch_size,
                          epochs=epochs,
                          callbacks=[checkpointer, csv_logger],
                          validation_data=generator_val_batch_sky(test_file, batch_size,num_classes,img_path),
