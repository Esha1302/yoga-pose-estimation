...
def dense201_hirar_sky(input_shape = (224,224,3),
        class_6=6):
    inputs = Input(input_shape)
    base_model= DenseNet201_hir(include_top=False, weights=None, input_tensor = inputs, backend = keras.backend , layers = keras.layers , models = keras.models , utils = keras.utils)
    
    x = base_model.output
    x = BatchNormalization( epsilon=1.001e-5, name = 'bn_class6_last')(x)
    x = Activation('relu', name='relu_class6_last')(x)
    x = GlobalAveragePooling2D(name='GAvgPool_class6_last')(x)
    
    x = Dense(class_6, activation='softmax')(x)
